...
proc<- procurements.plot(all, en=Sys.Date()+months(8))
...
p6.proc <- function(grouping.name, labor.by.week)
{
  ...
  p6.tmp<- labor.by.week %>% filter(week>=as.Date("2023-03-01")&week<=as.Date("2024-03-30")) %>%
...
ggplot(k$kr2, aes(x=d, colour=ctc)) + geom_step(aes(y=csum)) + facet_wrap(vars(grouping), scales="free_y")
```
