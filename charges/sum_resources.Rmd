...
all.charges <- bind_rows(bind_rows(bind_rows(c0,c1),c2),c3,c4)
...
FY.table <- data.frame(FY=c(2023,2024,2025,2026), frac=c(fract.of.year(fy23.interval),1.0,1.0,1.0))
...
# FY23 Q2
f.week<-as.Date('2023-01-01')
l.week<-as.Date('2023-04-01')
...
```{r}
  ...
  FY.code <- tmp %>%
    group_by(FY, code) %>% 
    summarise(hours=hours.to.FTE(frac,sum(dl.hours)),.groups='drop')
  FY.rsrc <- tmp %>%
    group_by(FY, rsrc) %>% 
    summarise(hours=hours.to.FTE(frac,sum(dl.hours)),.groups='drop')
  FY.type <- tmp %>%
  ...
  print(gtmp + geom_bar(aes(fill = rsrc)))
  print(gtmp + geom_bar(aes(fill = code)))
