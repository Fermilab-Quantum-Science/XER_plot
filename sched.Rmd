...
```{r}
# start date interval
start.interval <- interval('2023-03-01', '2023-07-30')
start.interval.Q4 <- interval('2023-09-01', '2023-12-30')
# start date interval
end.interval <- interval('2023-11-01', '2024-10-01')
late.interval <- interval('2025-09-01', '2025-12-31')
full.interval <- interval('2023-03-01', '2026-04-30')
...
```{r}
gp3<-ggplot(all.data%>%filter(type=='M' & target_end>as.Date('2023-10-01')& area<10 & area>1), 
  ...
  geom_text(aes(hjust='left',x = target_end, label = name), nudge_x=9.6, nudge_y=.4, size=1.8) +
...
print(gp3)
ggsave(gp3, filename="milestonesX.pdf",units="in", height=10, width=24, device="pdf")
```
...
getwd()
write.csv(
  all.data%>%filter(target_start>as.Date('2024-09-30')&target_end<as.Date('2025-10-01)'))%>%select(wbs, area,name, code, target_start, target_end, category, group),
...
write.csv(filter(all.data,area<11)%>%left_join(filter(udf.value,udf_type_id==3886), join_by(task_id==fk_id))%>%select(code, target_start, target_end, udf_text, area, wbs)%>%group_by(area,udf_text)%>%summarise(mi=min(target_start),ma=max(target_end))%>% arrange(ma),"in_kind_max_date.csv")
