---
title: "example-quarto2.qmd"
format: 
  beamer:
    df-print: kable
    tbl-colwidths: [15,55,15,15]
editor: visual
---
...
tmp.ims<-tmp.m%>%filter(str_detect(name,"^T0|^T1|^T2"))%>%select(code, name, target_end, actual_end, status, state, total_cost, task_id)
...
```{r}
#library(kableExtra)
tb2 <- tmp.both%>%filter(Tier=="T2" & Section=="S-1")%>%select(Activity, Name=name, Date=target_end,Status=act_end_date)%>% arrange(Date)
...
```{r}
tmp.both%>%filter(Tier=="T2" & Section=="S-2")%>%select(Activity, Name=name, Date=target_end, Status=act_end_date)%>% arrange(Date)
```
...
```{r}
tmp.both%>%filter(Tier=="T2" & Section=="S-3")%>%select(Activity, Name=name, Date=target_end, Status=act_end_date)%>% arrange(Date)
```
...
```{r}
tmp.both%>%filter(Tier=="T2" & Section=="S-4")%>%select(Activity, Name=name, Date=target_end, Status=act_end_date)%>% arrange(Date)
```
...
```{r}
tmp.both%>%filter(Tier=="T0")%>%select(Activity, Name=name, Date=target_end, Status=act_end_date)
```
...
```{r}
tmp.both%>%filter(Tier=="T1")%>%select(Activity, Name=name, Date=target_end, Status=act_end_date)
```
