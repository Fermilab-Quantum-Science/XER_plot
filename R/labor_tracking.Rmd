...
filt.bk<-filt.st - months(months.back)
...
p6.tasks <- all$all.data %>% 
  filter(target_start<=filt.en & target_end>=filt.bk) %>% 
  select(task_id,ctc, wbs, code, name, target_start, target_end, status,dur)
...
p6.recs <- p6.tasks %>% 
  ...
  select(Person=person, TaskDescription=name, TaskCode=ctc, WBS=wbs, 
         Activity=code, WorkingSchedStart=target_start, WorkingSchedEnd=target_end,
...
p6.reviews <- all$all.data %>% 
  ...
  select(Activity=code, Description=name,TaskCode=ctc, WBS=wbs,
         WorkingSchedStart=target_start, WorkingSchedEnd=target_end, 
         status) %>%
...
report.title <- data.frame(Name=c('ReportDate', 'PastDate', 'FutureDate',
                                  ...
                                  'Estimated Finish', 
                                  'Notes'),
                           Value=c(as.character(filt.st), as.character(filt.bk), as.character(filt.en),
