...
person.tasks.FY24<- labor$by.week.all %>%
  filter(FY %in% c(2024)) %>%
...
person.tasks.FY24.sum <- person.tasks.FY24 %>%
  group_by(Last.Name, quarter, task_id) %>%
  summarise(hrs=sum(dl.hours)) %>% 
  mutate(ftes=hrs/(1805)) %>%
  #filter(!is.na(person)) %>%
  #left_join(people%>%select(person=role_name, type=rsrc_name, DSC),join_by(type,person) ) %>%
...
#write_xlsx(person.tasks.FY24, paste(base.path,"tasks_FY24.xlsx",sep=''))
...
jbk.2 <- costs$target.by.week.task %>% ungroup() %>% 
  ...
  mutate(cumu.cost=cumsum(hsum)) %>%
  left_join(select(all$all.data, code, name, rsrc_type, status, target_start, target_end),by=join_by(code))
