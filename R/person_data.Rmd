...
persons.FY24Q1<- labor$by.week.person %>%
  filter(FY==2023)%>%
  filter(week %within% interval('2023-07-01','2024-02-12')) %>%
  group_by(person,FY)%>%
  summarise(hrs=sum(dl.hours)) %>% 
  mutate(ftes=hrs/(1805/4)) %>%
  filter(!is.na(person)) %>%
...
roles.table <- data.frame(a=c("Op Specialist", "Specialist", "Exp"),
                          b=c("Project Cntls", "Proj", "Prof"),
                          c=c("Eng Phys", "Specialist", "Exp"),
                          d=c("Particle Physicist", "Phys", "Phys"),
                          e=c("Mech Eng", "Eng", "Mech"),
                          f=c("PM", "Proj", "Proj"),
                          g=c("RA", "Phys", "Phys"),
                          h=c("Mech Design", "Design/Draft", "Mech"),
                          i=c("Mech Draft", "Design/Draft", "Mech"),
                          j=c("Mech Tech", "Tech", "Mech"),
                          k=c("Electrical Tech", "Tech", "Elec"),
                          l=c("Electrical Eng", "Eng", "Elec"),
                          m=c("App Developer", "Other", "Other"),
                          n=c("Metrologist", "Other", "Exp"),
                          o=c("Civil Eng", "Other", "ISD"),
                          p=c("Accel Sys Spec", "Specialist", "Exp"),
                          q=c("Construct Coord", "Other", "ISD"),
                          r=c("Electrical Task", "Tech", "Elec"),
                          s=c("Geodesist", "Other", "ISD"),
                          t=c("High Vac Tech", "Tech", "Exp"),
                          row.names=c("role", "role.type", "role.cat")
)

roles.table <- rownames_to_column(roles.table) %>% 
  pivot_longer(-rowname) %>% pivot_wider(names_from=rowname) %>% select (-name)
