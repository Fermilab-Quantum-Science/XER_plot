...
tmp.hr <- read_xlsx("../CostVarianceAnalysis/HoursJoined2023-10-012024-09-23_labeled.xlsx", sheet="Sheet3")%>% rename(person='Row Labels') 
...
# add ctc to this
# this file covered FY24 through Q3
tmp.hr.task <- read_xlsx("../CostVarianceAnalysis/HoursWeekTask2023-10-012024-07-01R.xlsx", sheet="Sheet1") %>%
...
tmp.hr.joined <- read_xlsx("../CostVarianceAnalysis/HoursJoined2023-10-012024-07-01R.xlsx", sheet="Sheet1") %>%
  filter(week>=filt.st, week<filt.en) %>%
  mutate(q=quarter(week, fiscal_start=10), y=fiscal.year(week), m=fiscal.month(week)) %>%
  mutate(month=floor_date(week,unit="month",week_start=1)) %>%
  mutate(quarter=floor_date(week,unit="quarter",week_start=1))
