...
write_xlsx(tpt.FY%>% filter(FY %in% c(2025, 2026)), paste(base.path,"output/TPT_FY25_26.xlsx",sep=''))
...
make.plot <- function(ds_names, use_cumu=TRUE)
{
  col_to_use = sym(if(use_cumu) "chours" else "hours")
  
  ggplot(tmp2.ra %>% 
                  filter(str_detect(Last.Name, ds_names))
                , aes(x=week, y=!!col_to_use, colour=src)) + 
  geom_line() +
...
make.plot(ADTech_names, use_cumu=FALSE)
```
...
make.plot(ADNotTech_names, use_cumu=FALSE)
```
