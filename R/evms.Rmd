...
fcosts <- data.frame(
  colors=c("grey70","grey70","grey80","grey80","grey90","grey90"),
  costs=c(15715738.00, 19731336.00, 15758634,15752059,15727715,15714400),
  names=c("PMP Planned Work", "PMP Guidance", "EAC Sep", "BAC Sep", "EAC Jun", "BAC Jun"),
...
```{r}
other <- get.all.schedule("BL2307")
other.costs <- bin.expenses(other$all.data)

WS.monthly <- costs$target.by.month  %>% 
  arrange(week) %>% group_by(week) %>% summarise(tot=sum(hsum)) %>%
  mutate(ctot=cumsum(tot)) %>%
  mutate(prerollup=week<as.Date("2023-03-03"), from="BL.2309") 

BL.monthly <- other.costs$target.by.month  %>% 
  arrange(week) %>% group_by(week) %>% summarise(tot=sum(hsum)) %>%
  mutate(ctot=cumsum(tot)) %>%
  mutate(prerollup=week<as.Date("2023-03-03"), from="PMP.Schedule") 

monthly=rbind(WS.monthly, BL.monthly)
```
...
print(g5d)
# width=12, height=10,
ggsave(g5d, filename=paste(base.path,"/output/evms.pdf",sep=''), units="in", device="pdf")
