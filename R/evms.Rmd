...
fcosts <- data.frame(
  colors=c("grey70","grey70","grey80","grey80","grey90","grey90","grey90","grey90"),
  costs=c(15715738.00, 19731336.00, 15718609,15691757, 15758634,15752059, 15727715,15714400),
  names=c("PMP Planned Work", "PMP Guidance", "EAC Dec", "BAC Dec", "EAC Sep", "BAC Sep", "EAC Jun", "BAC Jun"),
  xpos=c(min.dates, min.dates, max.dates, max.dates, max.dates, max.dates, max.dates, max.dates)
)
...
```{r}
baseline.current <- get.all.schedule("BCRBL2")
...
WS.monthly <- costs$target.by.month  %>% 
  ...
  mutate(prerollup=week<as.Date("2023-03-03"), from="2312.Forecast") 

BL.monthly <- baseline.current.costs$target.by.month  %>% 
  arrange(week) %>% group_by(week) %>% summarise(tot=sum(hsum)) %>%
  mutate(ctot=cumsum(tot)) %>%
  mutate(prerollup=week<as.Date("2023-03-03"), from="2310.Baseline") 
...
g5d<- ggplot(filt.data$months,
  ...
  geom_point(data=filt.data$fcprs%>%filter(!str_detect(name,"BAC|EAC")), aes(x=Period, y=value, colour=name), inherit.aes = FALSE, show.legend = FALSE) + 
