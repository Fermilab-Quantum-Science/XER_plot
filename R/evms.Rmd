...
```{r}
all <- get.all.schedule("WS2409")
all <- apply.baseline.schedule(all, "BL2409")
...
d.last.reporting.date <- as.Date("2024-09-30")
d.now = as.Date(now())
...
```{r}
baseline.current <- get.all.schedule("BL2409")
...
BL.monthly <- baseline.current.costs$target.by.month  %>% 
  arrange(week) %>% group_by(week) %>% summarise(tot=sum(hsum)) %>%
  mutate(ctot=cumsum(tot)) %>%
  mutate(prerollup=week<as.Date("2023-03-03"), from="2409.Baseline") 
