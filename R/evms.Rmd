...
```{r}
all <- get.all.schedule("WS2404")
all <- apply.baseline.schedule(all, "BL2404")
...
d.last.reporting.date <- as.Date("2024-03-31")
d.now = as.Date(now())
...
fcosts <- data.frame(
  colors=c("grey70","grey70","grey75","grey75",
           "grey80","grey80","grey90","grey90","grey90","grey90"),
  costs=c(15715738.00, 19731336.00, 16771287, 16150319, 
          16052283,15691757, 15915541,15752059, 15764280,15714400),
  names=c("PMP Planned Work", "PMP Guidance", "EAC Mar", "BAC Mar", 
          "EAC Dec", "BAC Dec", "EAC Sep", "BAC Sep", "EAC Jun", "BAC Jun"),
  xpos=c(min.dates, min.dates, max.dates, max.dates, 
...
```{r}
baseline.current <- get.all.schedule("BL2404")
...
WS.monthly <- costs$target.by.month  %>% 
  ...
  mutate(prerollup=week<d.last.reporting.date, from="2403.Forecast") %>%
...
WS.monthly <- WS.monthly %>% 
  add_row(week=floor_date(d.last.reporting.date,unit="month"),
          ...
          prerollup=TRUE,from="2403.Forecast" ,.before=1) %>%
  mutate(ctot=cumsum(tot))
...
BL.monthly <- baseline.current.costs$target.by.month  %>% 
  arrange(week) %>% group_by(week) %>% summarise(tot=sum(hsum)) %>%
  mutate(ctot=cumsum(tot)) %>%
  mutate(prerollup=week<as.Date("2023-03-03"), from="2403.Baseline") 
