...
gp3<-ggplot(
  ...
  # nudge_y=.4 was set for both the next ones
    geom_text(aes(hjust='left',x = target_end, label = paste(short_name, ' (', total_float,')', sep='')),
              nudge_x=9.6) +
    #geom_text(aes(hjust='right',x=as.Date(max(late_end))+months(3), label = paste(total_float, sep='/')),
    #          check_overlap = TRUE) +
    scale_x_date(date_breaks='3 month', date_labels = "%b %y", limits=c(as.Date("2023-03-01"), as.Date("2030-03-01"))) +
    theme(axis.text.x = element_text(angle = 90), legend.position = "top") +
    labs(x="Target End Date", y="Milestone", colour="State")

fac=.7
ggsave(gp3, filename=paste(base.path,'/output/milestones.pdf',sep=''),height=15*fac, width=14*fac,units='in', device="pdf")
#print(gp3)
