...
all.orig <- get.all.schedule("WS2412R")
...
#all <- apply.baseline.schedule(all.orig, "BL2409")
all <- apply.baseline.schedule(all.orig, "WS2412R")
...
activities<- activities.plot(all$all.data, start.date=as.Date('2024-09-01'),end.date=as.Date('2025-02-01'))
milestones<- milestones.plot(all$all.data, exclude.future=FALSE, earliest.date = as.Date("2024-08-01"), latest.date = as.Date("2025-10-01"))
resources<- resources.plot(labor$by.week, device="pdf")
proc<- procurements.plot(all, en=Sys.Date()+weeks(8*4))
reviews<- reviews.plot(all$all.data, start.date=as.Date("2024-10-01"), end.date=as.Date("2025-10-01"))
```
...
tmp.simpler <- all$all.data %>% select(code, name, total_float, target_start, target_end, wbs, wbs_high, wbs_low) %>% mutate(total_float=total_float/8) %>% filter(total_float < 40, target_start>=now()) %>% arrange(target_start, total_float, wbs) # %>% view()
...
tmp.simpler2 <- all$all.data %>% select(code, name, total_float, target_start, target_end, wbs, total_cost) %>% mutate(total_float=total_float/8) %>% filter(total_cost>100000, target_start>=now()) %>% arrange(target_start, total_float, wbs) %>% view("proc")
...
```{r}
tmp.simpler3 <- all$all.data %>% filter(target_start>=now(),str_detect(name,"Procure"),target_end<=as.Date("2026-01-07")) %>% select(code, name, total_float, target_start, target_end, wbs, total_cost, wbs_low) %>% mutate(total_float=total_float/8) %>%arrange(target_end) %>% view()
...
tmp.simpler4 <- all$all.data %>% select(code, name, total_float, target_start, baseline_start, baseline_end, target_end, wbs, wbs_high, wbs_low) %>% mutate(total_float=total_float/8)  %>% arrange(target_start, total_float, wbs) # %>% view()
tmp.variances <- read_xlsx("../variancesFY25Q1.xlsx")
